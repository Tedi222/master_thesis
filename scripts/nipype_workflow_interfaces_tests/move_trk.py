...
dti2mni_wf = Workflow(name='dti2mni_wf_split',
...
mni2dti2_wf = Workflow(name='mni2dti2_wf',
                       base_dir='/home/teodorps/tests/pipeline_tests/track_warp_nipype_testing')
...
mni2dti2_wf.connect([(sf, init_identity_warp, [('dwi', 'in_file')]),  # initialize identity warp
                  ...
                  (dti2mni_wf, tck2dwi, [('tck2mni.out_file', 'tracks')]),  # transform tracks from dwi to mni space
                  (mrtrix_dwi2mni_warp, tck2dwi, [('out_file', 'transform')])
                  ])

# map mrtrix streamlines on top of standard space to check if they were regstered to MNI correctly
mni2dti2_wf.connect([(tck2dwi, tckmap_mni2dti, [('out_file', 'in_file')]),
                  (sf, tckmap_mni2dti, [('dwi', 'reference')])
                  ])

# convert trk back to tck
mni2dti2_wf.connect([(dti2mni_wf, trk2tck, [('tck2trk.out_file', 'tractogram')])
                     ])

mni2dti2_wf.write_graph(graph2use='orig', format='svg')
mni2dti2_wf.write_graph(graph2use='orig', format='png')

mni2dti2_wf.write_graph(dotfilename='graph_colored.dot', graph2use='colored', format='svg')
mni2dti2_wf.write_graph(dotfilename='graph_colored.dot', graph2use='colored', format='png')
mni2dti2_wf.run()