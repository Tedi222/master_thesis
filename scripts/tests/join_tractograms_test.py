...
def join_tractograms(track_path_list):
    assert len(track_path_list) > 1, f'You must provide at least 2 tractograms, provided {len(track_path_list)}'
...
def main():
    track_folder_path = '../qbx_pl_impl_trk/centroids'
    track_paths = get_track_paths(track_folder_path)
    filtered_tracks = join_tractograms(track_paths)
    out_path = os.path.join(track_folder_path, 'output/merged.trk')
    if not os.path.exists(track_folder_path + '/output'):
        os.makedirs(track_folder_path + '/output')
    save_tractogram(filtered_tracks, out_path, bbox_valid_check=True)
