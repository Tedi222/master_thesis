...
tck2mni = Node(TCKTransform(out_file='tck2mni_tracks.tck'),
               name='tck2mni')
...
tck2dwi = Node(TCKTransform(out_file='tck2dwi_tracks.tck'),
               name='tck2dwi')
...
def main(subj_list_, cfg_):
    ...
    # pipeline
    pipeline = Workflow(name=cfg_['pipeline_name'], base_dir='/home/teodorps/filtering_results')
    ...
    pipeline.connect([(sf, convertwarp_dti2mni, [('standard', 'reference')]  # merge warps dti2t1 + t12mni
                        ...
                        (dwi2t1_nl, convertwarp_dti2mni, [('fieldcoeff_file', 'warp1')]),
                        (t12mni_nl, convertwarp_dti2mni, [('fieldcoeff_file', 'warp2')]),
                        ...
                        (tckcompress, tck2mni, [('out_file', 'tracks')]),  # transform tracks from dwi to mni space
    ...
    pipeline.connect([(tck2trk, cluster_streamlines, [('out_file', 'mni_tractogram_path')])
                      ])
    ...
    pipeline.connect([(sf, finta_filter, [('model', 'model'),
                      ...
                      (tck2trk, finta_filter, [('out_file', 'common_space_tractogram'),
                                                  ('out_file', 'original_tractogram')]),
    ...
    pipeline.connect([(finta_filter, trk2tck, [('out_filtered_trk', 'tractogram')])
                        ])

    pipeline.connect([(sf, init_identity_warp, [('dwi', 'in_file')]),  # initialize identity warp
                        (sf, mrtrix_dwi2mni_warp, [('standard', 'ref_file')]),
                        (init_identity_warp, mrtrix_dwi2mni_warp, [('out_file', 'in_file')]),  # create mrtrix warp
                        (convertwarp_dti2mni, mrtrix_dwi2mni_warp, [('out_file', 'field_file')]),
    ...
    # map mrtrix streamlines on top of standard space to check if they were regstered to MNI correctly
    pipeline.connect([(tck2dwi, tckmap_mni2dti, [('out_file', 'in_file')]),
    ...
    # FINTA
    pipeline.connect([
        (tck2dwi, tck2con_finta, [('out_file', 'in_file')]),
    ...
    for metric in ['fa', 'md', 'rd']:
        ...
        # sampled metrics
        pipeline.connect([
            (tck2dwi, tcksamplers_finta[metric], [('out_file', 'in_file')]),
            (dti2met, tcksamplers_finta[metric], [(image_name, 'in_image')]),
            (tck2dwi, tck2connecters_finta[metric], [('out_file', 'in_file')]),
    ...
    pipeline.connect([
        ...
        (dwi2fod_3tt, datasink, [('wm_odf' , 'dwi2fod_3tt.@wmodf')]),
        (dwi2response, datasink, [('wm_file', 'dwi2response.@wmfile')]),
        (dwi2response, datasink, [('gm_file', 'dwi2response.@gmfile')]), 
        (dwi2response, datasink, [('csf_file', 'dwi2response.@csffile')]),
        (tckcompress , datasink, [('out_file', 'tractogram.@full')]),
        (tcksift, datasink, [('out_file', 'tractogram.@filtered_sift')]),
        (tck2dwi, datasink, [('out_file', 'tractogram.@filtered_finta_dwi')]),

        (find_threshold, datasink, [('out_thresholds', 'finta_thres.@thresholds'),
                                    ('out_log', 'finta_thres.@threshold_log'),
                                    ('out_all', 'finta_thres.@graphs')]),
        (cluster_streamlines, datasink, [('streamline_classes_path', 'finta_cluster.@classes'),
                                         ('centroids_path', 'finta_cluster.@centroids_atlas'),
                                         ('clusters_valid_path', 'finta_cluster.@valid_clusters'),
                                         ('implausible_streamlines_path', 'finta_cluster.@implausible_trk')]),
        (finta_filter, datasink, [('out_filtered_trk', 'finta_filter.@mni_trk'),
                                  ('out_log', 'finta_filter.@filtering_logs'),
                                  ('out_all', 'finta_filter.@filtered_clusters_all')]),

        (tck2con_finta, datasink, [('out_file', 'connectivity_finta.@raw')]),
    ...
    #pipeline.run(plugin='MultiProc', plugin_args={'n_procs': 8, 'memory_gb': 32})
...
if __name__ == '__main__':
    ...
    subj_list = read_subj_list(cfg['subj_list'])[60:]#[20:37]#[51:60]#[37:54] #[1:3] #[:1]
    print(subj_list) 
    # Parallel(n_jobs=16)(delayed(main)([subj_], cfg) for subj_ in subj_list)
