...
def main(subj_list_, cfg_):
    ...
    pipeline.connect([
        ...
        # (find_threshold, datasink, [('out_thresholds', ' finta.@thresholds'),
        #                             ('out_log', 'finta.@threshold_log'),
        #                             ('out_all', 'finta.@graphs')]),
        # (cluster_streamlines, datasink, [('streamline_classes_path', ' finta.@classes'),
        #                                  ('centroids_path', 'finta.@centroids_atlas'),
        #                                  ('clusters_valid_path', 'finta.@valid_clusters'),
        #                                  ('implausible_streamlines_path', 'finta.@implausible_trk')]),
        # (finta_filter, datasink, [('out_filtered_trk', ' finta.@mni_trk'),
        #                           ('out_log', 'finta.@filtering_logs'),
        #                           ('out_all', 'finta.@filtered_clusters_all')]),
...
if __name__ == '__main__':
    ...
    subj_list = read_subj_list(cfg['subj_list'])#[1:2] #[:1]
    print(subj_list) 
    Parallel(n_jobs=10)(delayed(main)([subj_], cfg) for subj_ in subj_list)
